...
DROP INDEX IF EXISTS public.graph_nodes_embedding_idx;
DROP INDEX IF EXISTS public.graph_nodes_embedding_hnsw_idx;
DROP INDEX IF EXISTS public.graph_edges_embedding_idx;
DROP INDEX IF EXISTS public.graph_edges_embedding_hnsw_idx;
...
DROP FUNCTION IF EXISTS public.match_graph_nodes(vector, float, int, uuid);
DROP FUNCTION IF EXISTS public.match_graph_edges(vector, float, int, uuid);

ALTER TABLE public.document_chunks DROP COLUMN IF EXISTS embedding;
ALTER TABLE public.document_chunks ADD COLUMN IF NOT EXISTS embedding vector(1024);
