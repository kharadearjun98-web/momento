...
export function SignIn() {
  ...
  const onSubmit = async (data: SignInFormData) => {
    ...
    try {
      ...
      if (authData.session) {
        ...
        await ensureUserHasPlan(authData.session.user);
  ...
  return (
    <AuthLayout
      ...
      subheadline="Sign in to continue your learning journey."
    >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer group">
              <input
                type="checkbox"
                className="w-4 h-4 rounded border-gray-500 bg-[#1e1625]/80 text-purple-500 focus:ring-purple-500/50 focus:ring-offset-0"
              />
          ...
          <button
            ...
            className="btn-primary-glow mt-2 flex h-14 w-full items-center justify-center overflow-hidden rounded-xl text-white text-base font-bold font-display disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
